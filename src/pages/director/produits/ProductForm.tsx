...
const gammesAndFamilles = {
  "Batiment": ["peinture à base d'huile de lin", "laque", "protection et décoration", "peinture décorative"],
  "Aviation": ["laque bicomposant", "primaire de passivation bicomposant"],
  "Carrosserie": ["laque métallisée", "anti corrosion châssis", "mastic de finition", "laque de finition"],
  "Vernis": ["imprégnation séchage air", "imprégnation séchage four", "epoxy sol"],
  "Industrie": ["finition aluminium", "finition décorative", "finition brillante"],
};
...
export function ProductForm() {
  ...
  // Ajout des hooks d'état pour les listes
  const [gammes, setGammes] = useState<{ id: string, nom: string }[]>([]);
  const [familles, setFamilles] = useState<{ id: string, nom: string, gamme: string }[]>([]);
  const [emballages, setEmballages] = useState<{ id: string, nom: string }[]>([]);
  ...
  const [selectedEmballage, setSelectedEmballage] = useState<string>('');
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [gammesRes, famillesRes, emballagesRes] = await Promise.all([
          axios.get('/api/gammes/'),
          axios.get('/api/familles/'),
          axios.get('/api/emballages/'),
        ]);
        setGammes(Array.isArray(gammesRes.data) ? gammesRes.data : gammesRes.data.results || []);
        setFamilles(Array.isArray(famillesRes.data) ? famillesRes.data : famillesRes.data.results || []);
        setEmballages(Array.isArray(emballagesRes.data) ? emballagesRes.data : emballagesRes.data.results || []);
      } catch (err) {
        toast({ title: 'Erreur', description: 'Impossible de charger les listes', variant: 'destructive' });
      }
    }
    fetchData();
  }, [toast]);

  // Function to generate product reference
  const generateProductReference = (gamme: string, famille: string, emballage: string): string => {
    if (!gamme || !famille || !emballage) {
      return '';
    }

    const gammeLetterMap: { [key: string]: string } = {
      "Batiment": "B",
      "Aviation": "A",
      "Carrosserie": "C",
      "Vernis": "V",
    ...
    const gammeInitial = gammeLetterMap[gamme] || '';

    const familleIndex = gammesAndFamilles[gamme as keyof typeof gammesAndFamilles]?.indexOf(famille) + 1;
    
    const emballageCode = emballage === "seau-metallique" ? "1" : "2";

    if (gammeInitial && familleIndex && emballageCode) {
      return `${gammeInitial}${familleIndex}${emballageCode}01`;
    }
    return '';
  };

  useEffect(() => {
    if (selectedGamme && selectedFamille && watch('emballage')) {
      const newReference = generateProductReference(selectedGamme, selectedFamille, watch('emballage'));
      setValue('reference', newReference);
    } else {
      setValue('reference', '');
    }
  }, [selectedGamme, selectedFamille, watch('emballage'), setValue]);
  ...
  const onSubmit = (data: ProductFormData) => {
    ...
    // Ajout des IDs dans le FormData
    formData.append('gamme', selectedGamme);
    formData.append('famille', selectedFamille);
    formData.append('emballage', selectedEmballage);
    // Ajouter les autres champs comme avant
    Object.entries(data).forEach(([key, value]) => {
      if (['gamme', 'famille', 'emballage'].includes(key)) return; // déjà ajoutés
      if (value !== undefined) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent className="bg-white border border-gray-200 shadow-lg z-50">
                  {Array.isArray(familles) && familles.filter(f => f.gamme === selectedGamme).map((famille) => (
            ...
            <div className="space-y-3">
              ...
              <Select
                value={selectedEmballage}
                onValueChange={(value) => {
                  setSelectedEmballage(value);
                  setValue('emballage', value);
                }}
              >
                <SelectTrigger>
                ...
                <SelectContent className="bg-white border border-gray-200 shadow-lg z-50">
                  {Array.isArray(emballages) && emballages.map((emballage) => (
